...
Wrote the barebones logging lib. It has a view of an fd (doesn't handle
cleanup either) and just writes formatted output to it. Dead simple.

Tommorow I'll go through the git book submodules chapter then nest the lib into
seashell. Shouldn't have to change many call sites as the macros
...
- Don't have to specify TEST_SETUP and TEST_TEARDOWN everytime
...
The current mental model for the pty harness/debug view is as follows:

```
    pfd[0]-----------------seashell logs-----------------pfd[1]
     |                                                     ^
     v                                                     |
pty_harness<--->mfd<-----terminal io------>sfd<------->seashell
  |     ^
  |   pfd[0]
unity  |
output |
...
This should give the pty harness and debug view everything it needs from
seashell. Its output from the pty pair, and the logs from the pipe it set up
before execing.

After that all data would be cenrtralized. The pty harness would have data from
its own logs, seashells logs, seashell's pty output, and unity's output (see
UNITY_INCLUDE_CONFIG).

After that the debug view and the predicate functions would simply request
bytes from the buffer maintained by the pty and the debug view could read
directly from the log fd pipes, and the unity pipe.

My plan for now is to tie the life time of the debug view to the unity process.
So for every invocation of unity, it will stream all output (pty transcript,
logs, unity output, /proc based job table) to the ncurses display, and to
log files. This keeps things bounded memory wise. On UNITY_END I'll just
block to allow scrolling through the debug output.
